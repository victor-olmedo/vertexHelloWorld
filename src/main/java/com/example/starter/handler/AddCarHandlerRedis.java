  ...
  public void handle(RoutingContext rc){
    ...
    redis.dbsize()
      .onComplete(result -> {
        ...
        input.add(Integer.toString(result.result().toInteger()+1));
        input.add(attributes.get("car_make"));
        redis.set(input);
      });
    // Return that new car as a JSON file
    redis.dbsize().onComplete(result ->
      rc.json(new JsonObject().put(Integer.toString(result.result().toInteger()+1), attributes.get("car_make")))
    );
